...
public class TopologyBeanRW extends BasicCsvReader {
	...
	static CellProcessor[] writeProcessor = new CellProcessor[] { new ConvertNullTo("null"), new ConvertNullTo("null"),
			new ConvertNullTo("null"), new ConvertNullTo("null") };

	public static List<TopologyBean> readData(final BufferedReader bufferedReader) {
	...
	public static void writeData(final NetConfig netConfig, final File f, final String title) {
	...
	private static List<TopologyBean> convertToBeanList(final NetConfig netConfig) {
		List<TopologyBean> list = new ArrayList<TopologyBean>();
		Network net = netConfig.getNetwork();
		if (net.getFlatNet().isEmpty()) {
