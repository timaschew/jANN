	...
	public void doAction(final Actions a, final PropertyChangeEvent evt) {
		...
		case UPDATE_JUNG_GRAPH:
			...
			if ((Integer) evt.getOldValue() > newValue) {
				// remove last
				if (evt.getPropertyName().equals(SidebarConfig.P.hiddenNeurons.name())) {
					layerController.removeVertex(selectedHiddenLayer, newValue);
				} else if (evt.getPropertyName().equals(SidebarConfig.P.inputNeurons.name())) {
					layerController.removeVertex(0, newValue);
				} else if (evt.getPropertyName().equals(SidebarConfig.P.hiddenLayers.name())) {
					// layerController.removeLayer(maxHiddenLayer, newValue);
				} else if (evt.getPropertyName().equals(SidebarConfig.P.outputNeurons.name())) {
		...
		case UPDATE_SIDEBAR_TOPOLOGY_VIEW:
			...
			sidebar.topolgyPanel.inputSpinnerModel.setValue(sidebarConfig.getInputNeurons());
